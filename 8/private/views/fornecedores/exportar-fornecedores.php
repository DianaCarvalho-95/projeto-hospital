...
try {
    ...
    $colunas = [
        'Empresa', 'NIF', 'Telefone', 'Email', 'Morada', 'Website',
        'Pessoa de Contacto', 'Telefone de Contacto', 'Tipo de Fornecedor',
        'N.º Equipamentos', 'Observações'
    ];
    $linhas = [];
    ...
    if ($formato === 'csv') {
        enviar_csv('fornecedores', $colunas, $linhas);
    }
    if ($formato === 'imprimir') {
        enviar_impressao('Exportação de Fornecedores', $colunas, $linhas);
    }
    enviar_excel('fornecedores', 'Exportação de Fornecedores', $colunas, $linhas);
